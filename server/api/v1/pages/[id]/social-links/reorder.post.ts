...
interface SocialLinkOrder {
	id: string;
	order: number;
}

interface ReorderBody {
	linkOrders: SocialLinkOrder[];
}
...
export default defineEventHandler(async (event) => {
	...
	// 5. Validate all social links belong to this page
	...
	if (existingSocialLinks.length !== linkIds.length) {
		throw createError({
			statusCode: 400,
			message: 'Some social links do not belong to this page'
		});
	}
	...
	try {
		...
		console.error('Failed to update social link orders:', error);

		throw createError({
			statusCode: 500,
			message: 'Failed to update social link order'
		});
	}
});